...
# Load MiDaS model      Can change once using the jetson orin nano
model_type = "MiDaS_small"  # DPT_Large, DPT_Hybrid, MiDaS_small
...
def picture_in_picture(main_image, overlay_image, img_ratio=3, border_size=3, x_margin=30, y_offset_adjust=-100):
    ...
    x_offset = max(0, main_image.shape[1] - overlay_with_border.shape[1] - x_margin)
    y_offset = max(0, (main_image.shape[0] // 2) - overlay_with_border.shape[0] + y_offset_adjust)
...
while True:
    ...
    input_resized = cv2.resize(img, (256, 256))  # or (320, 240) if you want better quality
    input_batch = transform(input_resized).to(device)
    ...
    results = model.predict(image, verbose=False)  # predict on an image
    ...
    # Use OpenCV colormap for depth visualization
